...
def classify_words(word_dict):
    """词语分类,找出情感词、否定词、程度副词"""
    # 读取情感字典文件
    sen_file = io.open('BosonNLP_sentiment_score.txt', 'r+', encoding ='utf-8')
    # 获取字典文件内容
    ...
    # 读取否定词文件
    not_word_file = io.open('no.txt', 'r+', encoding='utf-8')
    # 由于否定词只有词，没有分值，使用list即可
    not_word_list = not_word_file.readlines()
 
    # 读取程度副词文件
    degree_file = io.open('degree.txt', 'r+', encoding='utf-8')
